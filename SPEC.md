...
- **Directive scope (name vs resource):** reconciliation matches a call against the human's directives on tool name AND argument scope, not the verb alone. A directive is either verb-level (`read_record`, any args) or resource-scoped (`read_record` for `record_id=42`). Verb-level authorizes any call to that tool by explicit choice — so it cannot tell a same-verb hijack (`read_record(43)`) from the authorized call; scope the directive to the resource and it can. This is a granularity decision owned by the authorizer, surfaced rather than buried: Crumb records against exactly what was authorized and never silently widens a directive. Same candor move as the federation trust set — we don't remove the "how tightly do I scope" decision, we make it explicit. (`hijack_demo` shows the sharp case: `read_record(43)` succeeds against a real record and is flagged `unauthorized` anyway.)
