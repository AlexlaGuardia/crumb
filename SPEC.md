...
- Multi-hop agent→agent→tool delegation is implemented for a SINGLE issuer (P6, RFC 8693 §4.1 nested `act` — `crumb/multihop_demo.py`). CROSS-issuer delegation (a chain spanning two IdPs) is still unsolved at the standards level — keep that boundary explicit, don't fake it.
